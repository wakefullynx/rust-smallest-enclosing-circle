...
#[component]
pub fn SmallestEnclosingCircleDemo() -> Element {
    let mut points = use_signal(|| Vec::from(INITIAL_POINTS));

    let smallest_circle = smallest_enclosing_circle::smallest_enclosing_circle(points());
                ...
                g {
                    {points().iter().enumerate().map(|(i, point)| rsx!{
                        circle {
                            ...
                            cy: point[1],
                            r: "1%",
                            class: format!("plain_point {}", if smallest_circle.is_on_circle(&point) { "spanning_point" } else { "" }),
                            onclick: move |event| {
